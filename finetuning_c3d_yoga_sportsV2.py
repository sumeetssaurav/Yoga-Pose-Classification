...
def plot_history(history, result_dir):
    ...
    plt.savefig(os.path.join(result_dir, 'model_accuracy_c3d_YogaV1_Split1_sportsV2.png'))
    plt.close()
    ...
    plt.savefig(os.path.join(result_dir, 'model_loss_c3d_yogaV1_Split1_SportsV2.png'))
    plt.close()
...
def save_history(history, result_dir):
    ...
    with open(os.path.join(result_dir, 'result_c3d_yogaV1_Split1_SportsV2.txt'), 'w') as fp:
...
def main():
    ...
    model.pop()
    #model.pop() ## For split-1 did this for rest two splits do not do this
    #model.pop()
    #model.pop()
    #model.pop()
    #model.pop()
...
# lets assume `model` is main model 
    ...
    save_best=ModelCheckpoint('models/weights_c3d_yogaV1_Split1_SportsV2.h5', monitor='val_accuracy', save_best_only=True)
    lr = 0.0001
    ...
    history = model.fit_generator(generator_train_batch(train_file, train_batch_size, num_classes,img_path),
                                  steps_per_epoch=train_samples // train_batch_size,
                                  epochs=epochs,
                                  callbacks=[onetenth_15_30_45(lr),save_best],
                                  validation_data=generator_val_batch(test_file,
                                        val_batch_size,num_classes,img_path),
                                  validation_steps=val_samples // val_batch_size,
                                  verbose=1)
    ...
    with open("c3d_yogaV1_Split1_SportsV2_json.json", "w") as json_file:
        json.dump(model_json, json_file)
    model.save_weights('results/weights_c3d_yogaV1_Split1_SportsV2.h5')
    steps = val_samples

    loaded_model = load_model('models/weights_c3d_yogaV1_Split1_SportsV2.h5')
